...
public class HealthItem extends Sprite {
    ...
    public void update() {
        if (isActive()) {
            // Move the health item (if needed)
            // Add any other logic for the health item's behavior

            // Check for collision with the player
            if (collidesWith(player.getBoundsInParent())) {
                // Increase player's health and deactivate the health item
                player.heal(HEALTH_INCREASE);
                setActive(false);
                removeFromPane(); // Remove the health item from the root
            }
        }
    }
