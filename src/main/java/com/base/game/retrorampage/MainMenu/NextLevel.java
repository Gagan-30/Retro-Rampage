...
public class NextLevel {
    ...
    public Scene createNextLevelScene(Scene previousScene, Stage mainStage) {
        try {
            // Initialize an FXMLLoader to load the FXML file associated with the NextLevel screen
            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("NextLevel-view.fxml"));
            ...
            NextLevelController nextLevelController = fxmlLoader.getController();
            // Pass the previous scene and the main stage to the controller for further use
            nextLevelController.setPreviousScene(previousScene);
            nextLevelController.setStage(mainStage);
